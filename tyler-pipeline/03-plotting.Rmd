...
plot(Sepal.Width ~ Sepal.Length, data = iris,
     ...
     col = adjustcolor(c("red", "blue", "orange")[as.numeric(Species)], alpha.f = 0.3))
```

## Real Data

```{r read otus}
samp1 <- read.table("Sample1.otu", sep = "\t", row.names = 1)
otus <- samp1[, ncol(samp1)]

samples <- t(samp1[, - ncol(samp1)])

taxTab <- strsplit(as.character(otus), split = ";  ")
taxTab <- do.call(rbind, taxTab)
taxTab <- gsub("^.__", "", taxTab)
...
library(RAM)
data(ITS1)
ITS1otus <- ITS1[, ncol(ITS1)]
ITS1samples <- t(ITS1[, - ncol(ITS1)])
ITS1taxTab <- strsplit(as.character(ITS1otus), split = "; ")
ITS1taxTab <- rbind.fill(lapply(ITS1taxTab, as.data.frame))
ITS1taxTab <- do.call(function(x) rbind.fill(as.data.frame(x)), ITS1taxTab)
ITS1taxTab <- gsub("^.__", "", ITS1taxTab)
colnames(ITS1taxTab) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
